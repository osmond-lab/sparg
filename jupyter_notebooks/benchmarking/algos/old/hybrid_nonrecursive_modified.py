        ...
        if all(child not in sub_cov_mats for child in skeleton[group]):
            ...
            for i, output_node_1 in enumerate(skeleton[group]):
                ...
                for j, output_node_2 in enumerate(skeleton[group]):
                    ...
                    #print(output_mat[i][j])
            aggregated_cov_mats[group] = np.bmat(output_mat)
        #for cov_mat in aggregated_cov_mats:
            #print("--", cov_mat)
            #print(aggregated_cov_mats[cov_mat])
        #print()
...
def calc_cov_matrix(ARG):
    ...
    for group in skeleton:
        ...
        child_list = list(edges.child)
        gmrca = ARG.ts.node(ARG.ts.num_nodes-1).id
        cov_mat = np.zeros((len(all_paths), len(all_paths)))
        for node in ARG.ts.nodes():
            if node.id != gmrca:
                path_indices = []
                for i, path in enumerate(all_paths):
                    if node.id in path:
                        path_indices.append(i)
                shared_time = ARG.ts.node(parent_list[child_list.index(node.id)]).time - node.time
                for a in path_indices:
                    for b in path_indices:
                        cov_mat[a, b] += shared_time
        sub_cov_mats[group] = cov_mat
        #print(all_paths)
