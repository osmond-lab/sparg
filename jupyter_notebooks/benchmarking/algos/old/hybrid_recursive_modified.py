    ...
    for group in skeleton:
        ...
        for pi in skeleton[group]:
            for pj in skeleton[group]: 
                cov_mat = np.zeros((len(paths[pi]), len(paths[pj])))
                for node in ARG.ts.nodes(): #this should be improved
                    if node.id != gmrca:
                        path_indices = []
                        for i, path in enumerate(paths[pi]):
                            if node.id in path:
                ...
                times[ (pi,pj) ] = np.matrix( cov_mat )
                times[ (pj,pi) ] = np.matrix( np.transpose(cov_mat) )
